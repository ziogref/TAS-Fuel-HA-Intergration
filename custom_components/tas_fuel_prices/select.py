...
async def async_setup_entry(
    ...
    user_fuel_types = entry.options.get(CONF_FUEL_TYPES, [])
    if user_fuel_types:
        # Sort the user-selected fuel types based on the predefined order
        sorted_fuel_types = sorted(
            user_fuel_types, 
            key=lambda x: FUEL_TYPE_ORDER.index(x) if x in FUEL_TYPE_ORDER else len(FUEL_TYPE_ORDER)
        )
        async_add_entities([FuelTypeSelect(entry, sorted_fuel_types)])
